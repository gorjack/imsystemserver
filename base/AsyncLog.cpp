...
{
    std::string pFileName = pszFileName;
    int nPos = pFileName.find_last_of("\\");
    std::string pLastStr = pFileName.substr(nPos + 1);
    ...
    snprintf(szFileName, sizeof(szFileName), "[%s:%d]", pLastStr.data(), nLineNo);
    strLine += szFileName;
...
{   
    ...
    snprintf(pszTime, nTimeStrLength, "%02d:%02d:%02d:%03d]", time.tm_hour, time.tm_min, time.tm_sec, tp.millitm);
