...
{
    std::string pLog(static_cast<const char *>(data), len);
    pLog.append("thread id= ");
    ...
    if (!channel_->isWriting() && outputBuffer_.readableBytes() == 0)
    {
        ...
        else // nwrote < 0
        {
            ...
            if (m_errorCallbackCB && errorCode == WSAEWOULDBLOCK && (time(NULL) - startTime) > m_nTimeOut)
            {
                msg = "超时";
                faultError = true;
                m_errorCallbackCB(msg);
            }
            else if (m_errorCallbackCB && errorCode != WSAEWOULDBLOCK)
            {
